...
class AuthControllerTest {
    ...
    @BeforeEach
    void setup() {
        User user = new User(1, "admin@admin.com", "P@ssw0rd!", true, List.of("ADMIN"));
    ...
    void shouldAuthenticateKnownUserAndReturn200() throws Exception {
        ...
        when(repository.findByUsername("admin@admin.com")).thenReturn(user);
    ...
    void shouldCreateValidUserAndReturn201() throws Exception {
        ...
        User user = new User(5, "user@user.com", "P@ssw0rd!", true, List.of("USER"));

        HashMap<String, Integer> createResult = new HashMap<>();
        createResult.put("userId", 5);

        String expectedJson = TestHelpers.serializeObjectToJson(createResult);

        when(repository.createUser(any())).thenReturn(user);
